...
# 4. Submission
Here is the final resul based on the prediction model "AccuracyEnsemblingAll" applied on the testing data set.

```{r Submission}
# Equalize testing columns with the features defined in the training models
        # If not, support vector machine model will fail
colNames <- colnames(subtesting)
colNames <- colNames[!(colNames %in% "classe")]
testing <- testing %>%
        select(one_of(colNames))

# Predict on testing (final) set
predictionDT_F <- predict(modFitDT1, newdata=testing, type="class")
predictionRF_F <- predict(modFitRF1, newdata=testing)
predictionBoost_F <- predict(modFitBoost1, newdata=testing)
predictionSupVect_F <- predict(modFitSupVect1, newdata=testing)


# Data combined
        # Ensembling on all vars

combinedPredictionDfAll_F <- data.frame(predictionDT_F, predictionRF_F, predictionBoost_F, predictionSupVect_F)



predictionEnsemblingAll_F <- predict(modFitEnsemblingAll1$finalModel, newdata=combinedPredictionDfAll_F)
predictionEnsemblingAll_F <- predict(modFitEnsemblingAll1, newdata=testing)
```
